...
while(True):
    ...
    if circles is not None:
        ...
        for i in circles[0,:]:
            ...
            cv2.circle(frame_out,(i[0],i[1]),i[2]-3,(int(r),int(g),int(b)),6)
            cv2.circle(frame_out,(i[0],i[1]),2,(int(r),int(g),int(b)),i[2])


        # Desenha a reta e calcula a distância entre os círculos
        if len(circ_list) >= 2:
            cv2.line(frame_out, circ_list[0], circ_list[1], (255,255,255), 2) # Linha entre círculos
            ...
            cv2.putText(frame_out, "Distance: %.2f cm" %real_dist, (5,fontsize*30), font, fontsize, (255,255,255))
            # print(real_dist)
            angle = np.arctan(dy/dx)*180/np.pi
            cv2.putText(frame_out, "Angle: %.4f" % angle, (5,fontsize*60), font, fontsize, (255,255,255))
            # print(angle)

    # Display the resulting frame
    cv2.imshow('Detector de circulos',frame_out)
